# Bölüm 2: Yazım Biçimi (Syntax)

Yeterince uzun süredir JS yazıyorsanız, yazım biçimine yeterince alışmışsınız demektir. Burada kesinlikle birçok tuhaflık göreceksiniz, fakat bu diğer dillerle benzerlik gösteren oldukça makul ve basit yazımlardır.

Ancak, ES6 alışmışın dışında olan yeni yazım biçimleri getirmiştir, bu bölümde bu biçimlerin neler olduğunu göreceğiz.

**Note:** Örnekleri Babel gibi dönüştürücüler ile online deneyerek pekiştirme yapabilirsiniz. [babeljs](http://babeljs.io/repl/)

## Block-Scoped Declarations

Belki fark etmişsiniz, Javascript değişkenleri için kapsam (scope) içerisine alırken `function` kullanırız. Eğer kampsam (scope) oluşturmanız gerekiyorsa bunun için en yaygın kullanılan yöntem kendi kendini çağıran fonksiyonlardır (IIFE). Örneğin:
...
Yinede, süslü parantaz içerisinde *block kapmsa* olarak tanımlama yapabiliriz. Bu demektir ki scope (kapsam) oluşturmak için tek ihtiyacımız olan bir çift parantez `{ .. }` ve `var` yerine `let` kullanlıyız ki, genel bir tanımlama yapmaktan kaçınabilelim.
...
***Uyarı:*** `ReferenceError` hatası `let` tanımlamasına erkenden erişildiği için fırlatılır, teknik olarak *Geçici Ölü Bölge (TDZ)* olarak adlandırılan bölümdedir. Tanımlama yapıldığı satır çalıştırılmadan erişilemez.

Son bir handikap ise: `typeof` TDZ içerisindeki `let` ile tanımlanan değişken için farklı davranır. Örneğin:
...
```js
{
	...
	// `b` tanımlandı, anca TDZ içerisinde
...
`a` hiç tanımlanmadı ve `typeof` ile var olup/olmama durumunu kontrol etmekte hiçbir problem yoktur anca `typeof b` için aynı işlem uygulandığında hata alırız. TDZ içerisinde olduğu için `let b` tanımlamasından önce `b` ile böyle bir işlem gerçekleştirilemiyor.

Şimdi açıkça neden `let` tanımlamalarını herzaman en üstte yapmamız gerektiğini daha iyi anlıyoruz. İleride farkında olmadan yapabileceğimzi hataların önüne geçmiş oluyoruz böylece.
